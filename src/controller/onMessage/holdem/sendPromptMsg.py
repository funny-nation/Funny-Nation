...
async def sendPromptMsg(channel: TextChannel, whoID: int or None, currentPot: int, amountForCall: int = 0, allInOnly: bool = False):
    if whoID is None:
        return
    currentPotMoneyDisplay = currentPot / 100
    if amountForCall != 0:
        if allInOnly:
            msg = f"轮到你了 <@{whoID}> 你只能选择全压或者弃牌，当前底池里有{currentPotMoneyDisplay}元。\n点击❌弃牌，点击⬆全压"
        else:
