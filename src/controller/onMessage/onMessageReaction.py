...
async def onMessageReaction(self: Client, reaction: Reaction, user: User, casino: Casino, db: Connection, gamePlayerWaiting: GamePlayerWaiting):
    ...
    for tableID in tables:
        if tables[tableID].isInviteMessage(reaction.message):
            ...
            await joinGameByReaction(table, user, reaction, self, db, casino, gamePlayerWaiting)
            break
